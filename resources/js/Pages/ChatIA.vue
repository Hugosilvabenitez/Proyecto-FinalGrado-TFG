...
// 🔹 Anchor al final del chat (clave)
const bottomRef = ref<HTMLElement | null>(null)

// 🔹 Scroll robusto estilo WhatsApp
...
// 🔹 Observamos SOLO la longitud (más fiable)
watch(
  () => messages.value.length,
  async () => {
    await nextTick()

    // 🔥 Garantiza que el DOM esté renderizado
    setTimeout(() => {
      scrollToBottom()
    }, 0)
  }
...
<template>
  ...
  <div class="h-screen bg-slate-950 text-slate-100 relative overflow-hidden flex flex-col overflow-hidden">
    ...
    <!-- Mensajes -->
    <div
      class="relative z-10 flex-1 overflow-y-auto px-4 py-6 space-y-4 max-w-3xl mx-auto w-full"
    >
      <div
    ...
    <!-- Input -->
    <div class="relative z-10 bg-white/5 backdrop-blur-2xl border-t border-white/10 px-4 py-3">
      <div class="max-w-3xl mx-auto flex gap-4 items-end px-4 py-3 bg-white/5 border border-white/10 rounded-2xl shadow-[0_0_20px_rgba(15,23,42,0.6)]">
        <textarea
          v-model="userInput"
