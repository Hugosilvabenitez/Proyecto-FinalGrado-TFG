...
* @date 2026-03-17
* 
* This file contains all the code for generate the forgot password area of Gameflux's project.
*/
...
<template>
    <GuestLayout>
        <Head title="Recuperar contraseña · GameFlux"/>
        <div class="min-h-screen flex items-center justify-center px-4 relative overflow-hidden bg-slate-950 text-slate-100">
            ...
            <div class="relative z-10 w-full max-w-md">
                ...
                <div class="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-2xl shadow-[0_0_40px_rgba(15,23,42,0.9)] p-8 transition duration-500 ease-out hover:border-cyan-400/60 hover:shadow-[0_0_60px_rgba(56,189,248,0.8)] hover:-translate-y-1 text-center">
                    <h1 class="text-2xl font-semibold tracking-tight mb-2">¿Olvidaste tu contraseña?</h1>
                    <p class="text-sm text-slate-300 mb-6">No te preocupes. Solo dinos tu email y te enviaremos un enlace para restablecer tu contraseña y volver a tus partidas.</p>
                    ...
                    <form @submit.prevent="submit" class="space-y-5">
                        <div>
                            ...
                            <TextInput id="email" type="email" class="mt-1 block w-full rounded-2xl bg-slate-900/60 border-slate-700/70 text-sm placeholder-slate-500 focus:border-cyan-400 focus:ring-cyan-400 text-center" v-model="form.email" required autofocus autocomplete="username" placeholder="trainer@gameflux.gg"/>
                    ...
                    <p class="mt-6 text-[11px] text-center text-slate-400">Revisa tu bandeja de entrada para recuperar el acceso a tu perfil de Game Boy Advance.</p>
                </div>
                <p class="mt-4 text-center text-sm text-slate-300">
                    <Link :href="route('login')" class="text-cyan-300 hover:text-cyan-100 font-medium">Volver al inicio de sesión</Link>
                </p>
            </div>
        </div>
    </GuestLayout>
